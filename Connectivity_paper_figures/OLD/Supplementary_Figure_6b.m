function Supplementary_Figure_6b()
dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
dir_current_fig = [dir_base  'Connectivity_paper_figures\plots\'];
filename = 'Supplementary_Figure_6c_short_distances';
...
rel_data_control = (STIMANAL.InfluenceDistanceIngermingledControl & 'flag_divide_by_std=0' & 'flag_withold_trials=1' & 'flag_normalize_by_total=1') ...
    &  (STIMANAL.SessionEpochsIncludedFinal& IMG.Volumetric & 'stimpower>=100' & 'flag_include=1'  ...
     & (STIMANAL.NeuronOrControlNumber & 'num_targets_neurons>=25') ...
    & (STIMANAL.NeuronOrControlNumber & 'num_targets_controls>=25'));
...
rel_control = rel_data_control & rel_sessions_neurons & key & sprintf('response_p_val=%.3f',response_p_val) & key_control;
rel_control_all = rel_data_control & rel_sessions_neurons & key & sprintf('response_p_val=1')  & key_control;
...
ax3=axes('position',[position_x1(1), position_y1(2), panel_width1, panel_height1]);
...
axes('position',[position_x1(1), position_y1(1)+0.23, panel_width1, panel_height1*0.5]);
hold on
% plot(xl, [0 0],'-k')
...
% shadedErrorBar(OUT1.distance_lateral_bins_centers,OUT2.marginal_lateral_mean,OUT2.marginal_lateral_stem,'lineprops',{'-','Color',[0 0.75 0.75]})
...
% text(150,0.004,sprintf('Control targets'),'Color',[0 0.75 0.75]);
% 
