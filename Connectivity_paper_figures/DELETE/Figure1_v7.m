...
DATA=fetch1(PAPER.ConnectivityPaperFigure1datav2,'figure_data');
lickmap_regular_odd_vs_even_corr_threshold=0.5; % will only affect panel m
%             information_per_spike_regular_threshold=0.1; % will only affect panel m
...
text(xl(1)+diff(xl)*0.5,yl(1)+diff(yl)*1.45,sprintf('Time to 1st contact-lick (s)'), 'FontSize',6,'HorizontalAlignment','center');
text(xl(1)+diff(xl)*0.5,yl(1)-diff(yl)*0.25,sprintf('Tongue position'), 'FontSize',6,'HorizontalAlignment','center','FontWeight','bold');
text(xl(1)-diff(xl)*0.6, yl(1)-diff(yl)*0.5, 'b', 'fontsize', 12, 'fontname', 'helvetica', 'fontweight', 'bold');
xlim(xl);
ylim(yl);
...
key_distance.odd_even_corr_threshold=0.5;
rel_data_distance= (LICK2D.DistanceCorrConcatSpikes *EXP2.SessionID & 'num_cells_included>=100') - IMG.Mesoscope;
rel_data_shuffled_distance= (LICK2D.DistanceCorrConcatSpikesShuffled *EXP2.SessionID & 'num_cells_included>=100') - IMG.Mesoscope;
