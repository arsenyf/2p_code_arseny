...
% key_tongue_trial1.trial=270; %% 218 207 %33
% 
...
plot(lickport_x_center,lickport_z_center,'ob','MarkerSize',10);
...
colormap_g=pink(numel(idx_licks_time_onset)*2);
 for i_l=1:1:numel(idx_licks_time_onset)
     idx_current_lick = idx_licks_time_onset(i_l):1: idx_licks_time_ends (i_l);
      plot(tongue_x_traj(idx_current_lick),tongue_z_traj(idx_current_lick),'-','Color',[colormap_g(i_l,:)],'LineWidth',1)
 end
plot(tongue_x_traj(idx_lick_contact_time),tongue_z_traj(idx_lick_contact_time),'.c','MarkerSize',5);
set(gca,'YDir', 'reverse');
xl=[-2,2];
xlim(xl);
yl=[0,4];
...
tongue_z_traj=fetch1(TRACKING.VideoBodypartTrajectTrial & key_tongue_trial2,'traj_z').*Zpix2mm.slope + Zpix2mm.intercept;
lickport_z_center=fetch1(TRACKING.VideoLickportPositionTrial  & key_tongue_trial2, 'lickport_z').*Zpix2mm.slope + Zpix2mm.intercept;
z_offset = nanmin(tongue_z_traj);
tongue_z_traj =  tongue_z_traj - z_offset;
lickport_z_center =  lickport_z_center- z_offset;

tongue_x_traj=fetch1(TRACKING.VideoBodypartTrajectTrial & key_tongue_trial2,'traj_x').*Xpix2mm.slope + Xpix2mm.intercept;
lickport_x_center=fetch1(TRACKING.VideoLickportPositionTrial  & key_tongue_trial2, 'lickport_x').*Xpix2mm.slope + Xpix2mm.intercept;
x_offset = nanmin(tongue_x_traj);
tongue_x_traj =  tongue_x_traj - x_offset;
lickport_x_center =  lickport_x_center- x_offset;
