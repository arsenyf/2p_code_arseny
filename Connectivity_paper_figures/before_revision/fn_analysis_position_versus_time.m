function fn_analysis_position_versus_time

rel_stats= LICK2D.ROILick2DPSTHStats & 'psth_odd_even_corr>0.5' & (IMG.ROIBrainArea & 'brain_area="MOp"');
key.subject_id = 464725;
rel_stats=rel_stats & key;

rel_stats2=LICK2D.ROILick2DmapStatsSpikes3bins  & rel_stats;

psth_time = fetch1(LICK2D.ROILick2DPSTH & key & rel_stats, 'psth_time','LIMIT 1');

peaktime_psth = fetchn(rel_stats & rel_stats2, 'peaktime_psth_regular','ORDER BY roi_number');
information_per_spike_regular = fetchn(rel_stats2, 'information_per_spike_regular','ORDER BY roi_number');
psth_position_concat_regular_odd_even_corr = fetchn(rel_stats2, 'psth_position_concat_regular_odd_even_corr','ORDER BY roi_number');
lickmap_regular_odd_vs_even_corr = fetchn(rel_stats2, 'lickmap_regular_odd_vs_even_corr','ORDER BY roi_number');
...
idx_positional = information_per_spike_regular>=0.2 & psth_position_concat_regular_odd_even_corr>0.5;
