function Supplementary_Figure_3_Reward_all_sessions___vsPoisson__sum
close all;

key = ((EXP2.Session & 'session>=0' ) & LICK2D.ROILick2DPSTHStatsSpikesPoisson & LICK2D.ROILick2DPSTHStatsSpikesResampledlikePoisson) - IMG.Mesoscope ;
...
if flag_spikes==1
    rel_all=LICK2D.ROILick2DPSTHStatsSpikes * LICK2D.ROILick2DPSTHStatsSpikesPoisson & key & rel_roi;
else
...
subplot(2,2,1)
rel_signif=  (rel_all   & 'reward_mean_pval_regular_large<0.05');
...
subplot(2,2,2)
rel_signif =  (rel_all   & 'reward_mean_pval_regular_small<0.05');
PSTH = fetch(rel_signif,'*');
