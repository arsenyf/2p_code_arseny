...
for i_sub = 1:1:numel(subject_list)
    ...
    for i_ses = 1:1:numel(sessions_list)
        ...
        x_tong_port_corr_before{i_sub,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
        ...
        lickport_z=zscore([T.lickport_z]);
        lick_peak_z=zscore([T.lick_peak_z]);
        
        x_tong_port_corr_after{i_sub,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
        ...
        x_tong_port_corr_first_touch{i_sub,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');

         T= fetch((TRACKING.VideoNthLickTrial*EXP2.SessionTrial*TRACKING.VideoLickportPositionTrial*EXP2.TrialLickPort & key )-TRACKING.VideoGroomingTrial & 'trial>100' & 'lick_number_relative_to_firsttouch>0','lick_peak_z','lickport_z');
        x_tong_port_corr_later_touch{i_sub,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
