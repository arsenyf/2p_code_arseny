function [start_file, end_file, lick_tr_times_relative_to_first_lick_after_go,lick_tr_total ] = fn_parse_into_trials_and_get_lickrate_alinged_to_Lickport (key, frame_rate, time_bin, flag_electric_video)
...
%% We align to the Lickport presentation. We only included trials for which there was also a contact lick  (there is some lag between GO cue and lickport movement). Detection can be based on either video or electric-lickport detection.
% Even if video detection option is chosen, but there is no video, we align based on electric lickport contact.  
go_time=fetchn(((EXP2.BehaviorTrialEvent & key) - TRACKING.VideoGroomingTrial) & 'trial_event_type="go"','trial_event_time'); % we take go_time as the beginning of the lickport movement
...
%         start_file(i_tr)=TrialsStartFrame(i_tr) + floor(licks_after_go(1)*frame_rate) + floor(time_bin(1)*frame_rate);
 start_file(i_tr)=TrialsStartFrame(i_tr) + floor(go_time(i_tr)*frame_rate) + floor(time_bin(1)*frame_rate); %instead we do it relative to GO (likcport ascend)
...
%         lick_tr_times_relative_to_first_lick_after_go{i_tr}=all_licks-licks_after_go(1);
         lick_tr_times_relative_to_first_lick_after_go{i_tr}=all_licks-go_time(i_tr); %instead we do it relative to GO (likcport ascend)
