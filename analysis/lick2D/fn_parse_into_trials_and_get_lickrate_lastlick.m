function [start_file, end_file, lick_tr_times_relative_to_first_lick_after_go,lick_tr_total ] = fn_parse_into_trials_and_get_lickrate_lastlick (key, frame_rate, time_bin, flag_electric_video)
...
%% We align to the last detected lick after Go cue (there is some lag between GO cue and lickport movement). Detection can be based on either video or electric-lickport detection.
...
for i_tr = 1:1:numel(trial_num)
    ...
    if ~isempty(licks_after_go)
        start_file(i_tr)=TrialsStartFrame(i_tr) + floor(licks_after_go(end)*frame_rate) + floor(time_bin(1)*frame_rate);
        end_file(i_tr)=start_file(i_tr)+floor([time_bin(2)-time_bin(1)]*frame_rate)-1;
        
        lick_tr_times_relative_to_first_lick_after_go{i_tr}=all_licks-licks_after_go(end);
