function [start_file, end_file, lick_tr_times_relative_to_first_lick_after_go,lick_tr_total ] = fn_parse_into_trials_and_get_lickrate_tonguemap (key, frame_rate, time_bin, flag_electric_video)

TrialsStartFrame=fetchn((IMG.FrameStartTrial  & key & TRACKING.VideoTongueTrial) - TRACKING.VideoGroomingTrial,'session_epoch_trial_start_frame','ORDER BY trial');
trial_num=fetchn((IMG.FrameStartTrial  & key & TRACKING.VideoTongueTrial) - TRACKING.VideoGroomingTrial,'trial','ORDER BY trial');
if isempty(TrialsStartFrame) % if its not mesoscope recording, but old recording (e.g. photostim) IMG.FrameStartTrial does not contain trial number, but in fact in behavioral session each file correspond to a new trial. 
    TrialsStartFrame=fetchn((IMG.FrameStartFile & key & TRACKING.VideoTongueTrial),'session_epoch_file_start_frame', 'ORDER BY session_epoch_file_num');
    trial_num=fetchn((EXP2.BehaviorTrial  & key & TRACKING.VideoTongueTrial) - TRACKING.VideoGroomingTrial,'trial','ORDER BY trial');
...
go_time=fetchn(((EXP2.BehaviorTrialEvent & key & TRACKING.VideoTongueTrial ) - TRACKING.VideoGroomingTrial) & 'trial_event_type="go"','trial_event_time');
LICK_ELECTRIC=fetch(((EXP2.ActionEvent & key & TRACKING.VideoTongueTrial) - TRACKING.VideoGroomingTrial),'*');
