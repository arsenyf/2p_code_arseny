function PLOT_InfluenceDistanceNoZscore()
dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
dir_current_fig = [dir_base  '\Photostim\Connectivity\'];
filename = 'Distance_Influence_no_zscore';
...
rel_data = (STIMANAL.InfluenceDistanceNoZscore & 'flag_divide_by_std=0' & 'flag_withold_trials=0' & 'flag_normalize_by_total=1') ...
...
caxis([OUT1.minv max(max(OUT1.map(:,1:end)))]);
colorbar
text(8, 0.1, ['Connection stength' newline '(\Delta DF/F activity)'],'Rotation',90);
axis off
