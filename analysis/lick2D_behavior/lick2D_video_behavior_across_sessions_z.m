function    lick2D_video_behavior_across_sessions_z()


dir_base = fetch1(IMG.Parameters & 'parameter_name="dir_root_save"', 'parameter_value');
dir_current_fig = [dir_base  '\Connectivity_paper_figures\plots\'];

filename=[sprintf('Supplementary_behavior_across_sessions_vertical')];
%% Graphics
%---------------------------------
figure;
...
for i_sub = 1:1:numel(subject_list)
    ...
    for i_ses = 1:1:numel(sessions_list)
        ...
        T= fetch((TRACKING.VideoNthLickTrial*EXP2.SessionTrial*TRACKING.VideoLickportPositionTrial*EXP2.TrialLickPort & key )-TRACKING.VideoGroomingTrial & 'trial>100' & 'lick_number_relative_to_lickport_entrance<=-1','lick_peak_z','lickport_z');
        x_tong_port_corr_before{subj_counter,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
...
%         early_licks_all{subj_counter,i_ses} = mean(early_licks_trials);
        ...
        T= fetch((TRACKING.VideoNthLickTrial*EXP2.SessionTrial*TRACKING.VideoLickportPositionTrial*EXP2.TrialLickPort & key )-TRACKING.VideoGroomingTrial & 'trial>100' & 'lick_number_relative_to_lickport_entrance>=0','lick_peak_z','lickport_z');
        x_tong_port_corr_after{subj_counter,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
        trials_list= unique([T.trial]);
        after_licks_all{subj_counter,i_ses} = numel(T)/numel(trials_list);

        T= fetch((TRACKING.VideoNthLickTrial*EXP2.SessionTrial*TRACKING.VideoLickportPositionTrial*EXP2.TrialLickPort & key )-TRACKING.VideoGroomingTrial & 'trial>100' & 'lick_number_relative_to_firsttouch=0','lick_peak_z','lickport_z');
        x_tong_port_corr_first_touch{subj_counter,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
        
        T= fetch((TRACKING.VideoNthLickTrial*EXP2.SessionTrial*TRACKING.VideoLickportPositionTrial*EXP2.TrialLickPort & key )-TRACKING.VideoGroomingTrial & 'trial>100' & 'lick_number_relative_to_firsttouch>0','lick_peak_z','lickport_z');
        x_tong_port_corr_later_touch{subj_counter,i_ses}=corr([T.lickport_z]',[T.lick_peak_z]','Rows','pairwise');
