...
for i_s=1:1:numel(unique_sessions_before)
    ...
    number_of_trials_in_session = numel(unique([T.trial]));
...
%     lickport_lickable_duration(i_s)=mean([T.lickport_lickable_duration]);
    ...
    [counts,edges_sessions]=histcounts(lick_time_onset,[-1.5:time_bin:10]);
   bins_seesions(i_s,:)=(counts./number_of_trials_in_session)/time_bin;  % we divide the count of licktimes for all trials by the number of trials and time bin
...
yl=[0,max([max(lick_time_mean),4])];
plot([mean(mean_lickport_t_entrance_min),mean(mean_lickport_t_entrance_min)],yl,'-r')
plot([mean(mean_lickport_t_entrance_max),mean(mean_lickport_t_entrance_max)],yl,'-r')
