...
classdef ConnectivityPaperFigure2datav2shuffleFemales < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_current_fig = [dir_base  'Connectivity_paper_figures\plots\females\'];
            
            filename=[sprintf('Figure2v4_females')];
            ...
            rel_roi=PAPER.ROILICK2DInclusion & (LAB.Subject & 'sex="F"');
            ...
            rel_positional_signif = rel_roi & (LICK2D.ROILick2DmapSpikes3binsPvalue2 & sprintf('lickmap_fr_regular_modulation_pval<=%.2f',p_value_threshold));
            ...
            rel_positional_and_allreward_signif = rel_roi & (LICK2D.ROILick2DmapSpikes3binsPvalue2 & sprintf('lickmap_fr_regular_modulation_pval<=%.2f',p_value_threshold))...
                & (rel_reward_large_signif | rel_reward_small_signif);
            
             rel_temporal_and_allreward_signif = rel_allreward_signif & rel_temporal_signif;
            ...
            rel_stats222= LICK2D.ROILick2DmapSpikes3binsPvalue2*IMG.ROIID & rel_roi & rel_allreward_signif ;
            ...
            D_reward.lickmap_fr_regular_modulation_pval =fetchn(rel_stats222, 'lickmap_fr_regular_modulation_pval','ORDER BY roi_number_uid');
            ...
            D_venn_reward_positional.count_reward_and_positional = count(rel_positional_and_allreward_signif);
            ...
            D_venn_reward_temporal.count_total=count(rel_roi);
            D_venn_reward_temporal. count_reward_and_temporal = count(rel_temporal_and_allreward_signif);
            D_venn_reward_temporal.A_count_largereward=count(rel_reward_large_signif);
            D_venn_reward_temporal.B_count_smallreward=count(rel_reward_small_signif);
            D_venn_reward_temporal.C_count_temporal=count(rel_temporal_signif);
            D_venn_reward_temporal.AB= count(rel_reward_large_signif & rel_reward_small_signif);
            D_venn_reward_temporal.AC= count(rel_reward_large_signif & rel_temporal_signif);
            D_venn_reward_temporal.BC= count(rel_reward_small_signif & rel_temporal_signif);
            D_venn_reward_temporal.ABC= count(rel_reward_large_signif & rel_reward_small_signif & rel_temporal_signif);
            D_venn_reward_temporal.venn_labels={'A_count_largereward', 'B_count_smallreward', 'C_count_temporal'};

            
            key.figure_data.D_venn_reward_positional=D_venn_reward_positional;
            key.figure_data.D_venn_reward_temporal=D_venn_reward_temporal;
