...
classdef CorrPairwiseDistanceSVDPaper < dj.Computed
    properties
%         keySource =(EXP2.SessionEpoch & IMG.ROIdeltaF & STIMANAL.MiceIncluded & IMG.PlaneCoordinates)- EXP2.SessionEpochSomatotopy-IMG.Mesoscope &  (STIMANAL.SessionEpochsIncludedFinalUniqueEpochs & IMG.Volumetric & 'stimpower>=100' & 'flag_include=1');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            flag_zscore=0; %0 only centering the data (i.e. mean subtraction),  1 zscoring the data
            rel_data = (IMG.ROIdeltaF & key)-IMG.ROIBad;
            for it=1:1:numel(time_bin_vector)
                fn_compute_CorrPairwiseDistancePaper(key, rel_data, self, flag_zscore,time_bin_vector(it), threshold_for_event_vector, dir_save_fig, num_svd_components_removed_vector)
            end
        end
    end
end
