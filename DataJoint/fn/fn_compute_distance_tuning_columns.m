function fn_compute_distance_tuning_columns(rel_roi, rel_data, key,self, dir_save_fig, rel_roi_xy,mesoscope_flag)
...
lateral_distance_bins=[10, 20,30:10:500];
eucledian_distance_bins=[10, 20,30:10:500];
...
try
    filename = sprintf('anm%d_s%d_%s_oddeven_threshold%d_modulationthreshold%d',key.subject_id,key.session, session_date, 100*key.odd_even_corr_threshold, key.modulation_threshold);
catch
...
F=100*F./nanmax(F,[],2); %normalizing each tuning by its max

%% Computing average modulation depth of cells within a certain radius, lateral distances

modulationdepth_radius_cell=[];
for i_l=1:1:numel(lateral_distance_bins)-1
    population_modulationdepth_neighborhood=zeros(numel(x_all),1);
    population_modulationdepth_neighborhood_shuffle =zeros(numel(x_all),1);
    parfor iROI=1:1:numel(x_all)
        idx_lateral_radius_roi = dXY(iROI,:)<=lateral_distance_bins(i_l+1);
        population_tuning_neighborhood = nanmean(F(idx_lateral_radius_roi,:),1); % population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance. Population tuning is the average of the tuning curves of all included cells in the neihborhood
        population_modulationdepth_neighborhood(iROI) = fn_compute_generic_1D_or_2D_tuning_modulation_depth (population_tuning_neighborhood); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance
        %shuffle
        idx_randpermute = idx_lateral_radius_roi([1,randperm(numel(idx_lateral_radius_roi(2:end)))+1]);
        population_tuning_neighborhood_shuffle = nanmean(F(idx_randpermute,:),1); % population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance. Population tuning is the average of the tuning curves of all included cells in the neihborhood
        population_modulationdepth_neighborhood_shuffle(iROI) = fn_compute_generic_1D_or_2D_tuning_modulation_depth (population_tuning_neighborhood_shuffle); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance
    end
    neighborhood_modulation_lateral_radius (i_l) =nanmean(population_modulationdepth_neighborhood); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance, averaged across all cells
    neighborhood_modulation_lateral_shuffle_radius (i_l) =nanmean(population_modulationdepth_neighborhood_shuffle); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance, averaged across all cells
end


figure
subplot(2,2,1)
plot(lateral_distance_bins(1:end-1),neighborhood_modulation_lateral_radius,'-r')
hold on
plot(lateral_distance_bins(1:end-1),neighborhood_modulation_lateral_shuffle_radius,'-k')
xlabel('Neighborhood radius Lateral distance (um)')
ylabel('Neighborhood Modulation (%)')
title(sprintf('Modulation=%.1f %% Stability = %.2f', key.modulation_threshold, key.odd_even_corr_threshold ));


%% Computing average modulation depth of cells within a certain radius, lateral distances

modulationdepth_radius_cell=[];
for i_l=1:1:numel(eucledian_distance_bins)-1
    population_modulationdepth_neighborhood=zeros(numel(x_all),1);
    population_modulationdepth_neighborhood_shuffle =zeros(numel(x_all),1);
    parfor iROI=1:1:numel(x_all)
        idx_lateral_radius_roi = d3D(iROI,:)<=eucledian_distance_bins(i_l+1);
        population_tuning_neighborhood = nanmean(F(idx_lateral_radius_roi,:),1); % population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance. Population tuning is the average of the tuning curves of all included cells in the neihborhood
        population_modulationdepth_neighborhood(iROI) = fn_compute_generic_1D_or_2D_tuning_modulation_depth (population_tuning_neighborhood); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance
        %shuffle
        idx_randpermute = idx_lateral_radius_roi([1,randperm(numel(idx_lateral_radius_roi(2:end)))+1]);
    ...
    neighborhood_modulation_eucledian_radius (i_l) =nanmean(population_modulationdepth_neighborhood); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance, averaged across all cells
    neighborhood_modulation_eucledian_shuffle_radius (i_l) =nanmean(population_modulationdepth_neighborhood_shuffle); % modulation depth of population tuning in the neighborhood of each cell, with neihborhood defined by lateral distance, averaged across all cells
end



subplot(2,2,2)
plot(eucledian_distance_bins(1:end-1),neighborhood_modulation_eucledian_radius,'-r')
hold on
plot(eucledian_distance_bins(1:end-1),neighborhood_modulation_eucledian_shuffle_radius,'-k')
xlabel('Neighborhood radius Eucledian distance (um)')
ylabel('Neighborhood Modulation (%)')
title(sprintf('Modulation=%.1f %% Stability = %.2f', key.modulation_threshold, key.odd_even_corr_threshold ));
...
% hold on
% plot(lateral_distance_bins(1:end-1),modulation_depth_all_avg)

% modulationdepth_radius_cell=zeros(numel(axial_distance_bins),numel(lateral_distance_bins)-1);
% for i_l=1:1:numel(lateral_distance_bins)-1
...
%         modulationdepth_radius_cell(i_a,i_l)=nanmean(rho_lateral(idx_axial));
%     end
% end
...
key.neighborhood_modulation_lateral=neighborhood_modulation_lateral_radius;
key.neighborhood_modulation_lateral_shuffle=neighborhood_modulation_lateral_shuffle_radius;
key.neighborhood_modulation_eucledian=neighborhood_modulation_eucledian_radius;
key.neighborhood_modulation_eucledian_shuffle=neighborhood_modulation_eucledian_shuffle_radius;
