function   fn_compute_graph_and_connectivity_stats(key, p_val_threshold, minimal_distance, dir_save_figure, self)
close;
...
rel_roi = rel_roi_temp & rel_group;
...
title(sprintf('Session %d epoch %d \n anm %d  %s \n In-Out degree corr = %.2f unidirectional proportion = %.2f \n p val=%.3f',s.session,  key.session_epoch_number,s.subject_id, s.session_date,in_out_degree_correlation, unidirectional_proportion, p_val_threshold ),'FontSize',12);
