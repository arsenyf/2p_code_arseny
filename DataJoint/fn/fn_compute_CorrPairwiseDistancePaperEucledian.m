function fn_compute_CorrPairwiseDistancePaperEucledian(key, rel_data, self, flag_zscore,time_bin, threshold_for_event_vector, dir_save_fig, num_svd_components_removed_vector)

rel_roi = (IMG.ROI - IMG.ROIBad) & rel_data;
...
distance_bins=[0:25:200,250:50:450,500:100:1000]; % in microns
...
pix2dist=  fetch1(IMG.Zoom2Microns & key & kkk,'fov_microns_size_x') / fetch1(IMG.FOV & key, 'fov_x_size');
rel_roi_xy = (IMG.ROIPositionETL)  & rel_roi; % XYZ coordinate correction of ETL abberations based on ETL callibration
% to correct for ETL abberations
x_all = fetchn(rel_roi_xy ,'roi_centroid_x_corrected','ORDER BY roi_number')*pix2dist;
y_all = fetchn(rel_roi_xy ,'roi_centroid_y_corrected','ORDER BY roi_number')*pix2dist;
z_all=fetchn(rel_roi*IMG.ROIdepth,'z_pos_relative','ORDER BY roi_number');
...
for i_th = 1:1:numel(threshold_for_event_vector)
    ...
    for i_c = 1:1:numel(num_svd_components_removed_vector)
        ...
        kk=fn_POP_bin_pairwise_corr_by_distance(key, rho, d3D, threshold_for_event_vector, i_th,i_c, corr_histogram_bins, distance_bins, num_svd_components_removed_vector);
