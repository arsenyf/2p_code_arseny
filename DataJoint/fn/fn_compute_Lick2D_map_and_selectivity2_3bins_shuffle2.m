function fn_compute_Lick2D_map_and_selectivity2_3bins_shuffle2 (key,self, rel_data, fr_interval, fr_interval_limit, flag_electric_video, self2)
number_of_bins=3;
...
% introducing jitter in the bins. This is used in case the original grid is not a
% perfect 3 by 3 grid, and in this way we can assign the borderline bins  more evenly across bins
...
for i_roi=1:1:size(S,1)
    ...
    for i_shuffle = 1:1:num_shuffles
        if i_shuffle==1 % first iteration of the shuffle we don't actually do shuffle but use the original data
            ...
            shifted_trial_order = randperm(num_trials);
            
            counter=0;
            for i_x=1:1:numel(x_bins_centers)
                
                for i_z=1:1:numel(z_bins_centers)
                    num_trials_to_shift = numel(idx_xz{i_z,i_x});
                    idx_xz_shuffled{i_z,i_x} = shifted_trial_order(counter+1:(counter+num_trials_to_shift));
                    counter = counter + num_trials_to_shift;
                end
            end
        end
        ...
        for i_x=1:1:numel(x_bins_centers)
            for i_z=1:1:numel(z_bins_centers)
                ...
                idx_bin = idx_xz_shuffled{i_z,i_x};
