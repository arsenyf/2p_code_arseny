function [psth_time] = fn_plot_single_cell_psth_by_position_example_with_stability (rel_example, roi_number_uid, xlabel_flag, ylabel_flag, position_x1_grid, position_y1_grid, cell_number2d)
...
panel_width1_stability_map=0.02;
panel_height1_stability_map=0.02;
...
ax6=axes('position',[position_x1_grid(1)+0.09,position_y1_grid(1)+0.03, panel_width1_stability_map, panel_height1_stability_map]);
mmm=P.lickmap_fr_regular_odd;
% we do it for display purpose only only for those experiment for which there was no data in one of  the positions due to technical reasons
mmm= fn_map_2D_legalize_by_neighboring_psth(mmm);
...
if ylabel_flag==1 & xlabel_flag==0
    title(sprintf('Location tuning stability:               \n Odd trials'), 'FontSize',6);
else
    title(sprintf('\n Odd trials'), 'FontSize',6);
end
axis equal;
...
ax6=axes('position',[position_x1_grid(1)+0.09,position_y1_grid(1), panel_width1_stability_map, panel_height1_stability_map]);
