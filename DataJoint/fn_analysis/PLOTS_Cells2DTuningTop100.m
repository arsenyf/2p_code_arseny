function PLOTS_Cells2DTuningTop100 (key, dir_current_fig, flag_spikes, plot_one_in_x_cell , rel_rois)
close all;
...
M = M(idx_sort,:);
M_poisson = M_poisson(idx_sort,:);
M_resampled_likepoisson = M_resampled_likepoisson(idx_sort,:);
roi_number=roi_number(idx_sort);

roi_number=roi_number(idx_sort);

roi_uid=roi_uid(idx_sort);

for i_roi=1:plot_one_in_x_cell:100%numel(roi_number)
