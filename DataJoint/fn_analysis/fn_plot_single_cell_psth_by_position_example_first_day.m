function [psth_time] = fn_plot_single_cell_psth_by_position_example_first_day (rel_example, roi_number_uid, xlabel_flag, ylabel_flag, position_x1_grid, position_y1_grid, cell_number2d)
...
panel_width_map=0.02;
panel_height_map=0.02;
    ...
    for  i_z=1:1:number_of_bins
        ...
        if current_plot ==1
            if ~isempty(cell_number2d)
                ...
                text(-2,6.5,sprintf('Cell %d',cell_number2d),'HorizontalAlignment','left', 'FontSize',6, 'fontweight', 'bold');
            end
            if xlabel_flag==1
