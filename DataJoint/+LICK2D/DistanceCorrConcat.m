...
classdef DistanceCorrConcat < dj.Computed
    properties
                keySource = (EXP2.SessionEpoch  & IMG.ROI & LICK2D.ROILick2Dmap3bins - IMG.Mesoscope) & IMG.Volumetric ;
    end
    methods(Access=protected)
        function makeTuples(self, key)
           ...
           if mesoscope_flag==1
                dir_save_fig = [dir_base  'Lick2D\population\TUNING_vs_DISTANCE\single_sessions\tuning_by_concatenated_psth\dff\'];
            else
                dir_save_fig = [dir_base  '\Lick2D\population\TUNING_vs_DISTANCE\single_sessions_mesoscope\tuning_by_concatenated_psth\dff\'];
            end
            
            for i_c = 1:1:numel(psth_position_concat_regular_odd_even_corr)
                %this is just the inclusion criteria
                ...
                rel_data = LICK2D.ROILick2Dmap3bins & rel_roi & key;
