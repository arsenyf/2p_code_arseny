...
classdef InfluenceDistanceIngermingledControl < dj.Computed
    properties
        keySource = (EXP2.SessionEpoch & 'flag_photostim_epoch =1') & STIM.ROIResponseDirectUnique & (STIMANAL.SessionEpochsIncludedFinal & 'flag_include=1') & STIMANAL.ControlTargetsIntermingled;
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            pval=[0.01, 0.05, 1];
            distance_vector1 = [25,50,100,5000]; %distance to closest target of control sites
            flag_withold_trials = [0,1];
            flag_normalize_by_total = [1];
            flag_divide_by_std = [0]; % 1 divide the response by std. 0 don't divide
            
            for i_n = 1:1:numel(distance_vector1)
                distance_to_closest_responsive_target = distance_vector1(i_n);
                rel_direct= STIMANAL.ControlTargetsIntermingled & sprintf('distance_to_closest_responsive_target<=%2f',distance_to_closest_responsive_target) & key;
                        ...
                        for i_total = 1:1:numel(flag_normalize_by_total)
                            for i_w = 1:1:numel(flag_withold_trials)
                                for i_std = 1:1:numel(flag_divide_by_std)
                                    ...
                                    k.distance_to_closest_responsive_target = distance_to_closest_responsive_target;
