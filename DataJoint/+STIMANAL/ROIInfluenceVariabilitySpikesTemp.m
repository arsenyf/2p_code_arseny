...
classdef ROIInfluenceVariabilitySpikesTemp < dj.Computed
    properties
        ...
        keySource = EXP2.SessionEpoch & STIMANAL.ROIResponseDirectVariabilitySpikes & 'subject_id=462458' & 'session=5';
...
%             for i_g=1:1:numel(group_list)
                ...
                response_trials_direct = fetchn(STIMANAL.ROIResponseDirectVariabilitySpikes & key,'response_peak_trials');
                ...
                response_trace_trials_direct = fetch1(STIMANAL.ROIResponseDirectVariabilitySpikes & key,'response_trace_trials');
                time_vector_direct = fetch1(STIMANAL.ROIResponseDirectVariabilitySpikes & key,'time_vector');
                
                
                
                
                for i_r = find(signif_roi==1067)
%                                     for i_r = 1:1:numel(signif_roi)
                    ...
                    kk_insert.plane_num = DATA(i_r).plane_num;
                                        ...
                                        num_weak_trials =sum(idx_weak);
                                        num_strong_trials =sum(idx_strong);
                                        num_all_trials =length(idx_strong);
...
%                     title([sprintf('''Pre-synaptic''')]);
                    ...
                    [StimStat,StimTrace] = fn_compute_photostim_response_variability (f_trace_coupled , photostim_start_frame, timewind_response, timewind_baseline1,timewind_baseline2,timewind_baseline3, flag_baseline_trial_or_avg, global_baseline, time);
