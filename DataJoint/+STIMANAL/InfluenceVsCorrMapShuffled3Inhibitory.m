...
classdef InfluenceVsCorrMapShuffled3Inhibitory < dj.Computed
    properties
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            maximal_distance=250; %um lateral;  exlude all cells further than maximal distance from target
            
            % bins
            bins_corr = linspace(-1,1,6); % if there is no SVD component/s subtraction
%             bins_influence = [-inf, 0, 0.25, 0.5, 0.75, inf];
                        bins_influence = [-inf,linspace(-0.2,0,5)];
            ...
            dir_fig = [dir_base  '\Photostim\Connectivity_vs_Tuning\Inhibitory\single_sessions\tuning_by_map_stability\'];
