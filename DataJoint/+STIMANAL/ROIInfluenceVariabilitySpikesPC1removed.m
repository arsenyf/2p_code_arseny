...
classdef ROIInfluenceVariabilitySpikesPC1removed < dj.Computed
    properties
        ...
        keySource = EXP2.SessionEpoch & STIMANAL.ROIResponseDirectVariabilitySpikesPC1removed &  (STIMANAL.SessionEpochsIncludedFinalUniqueEpochs & IMG.Volumetric & 'stimpower>=100' & 'flag_include=1');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            dir_current_fig = [dir_base  '\Photostim\photostim_traces\coupled_analysis_for_ilan_paper_spikes\'];
            ...
            k_pc.num_svd_components_removed=1;
            ...
            for i_g=1:1:numel(group_list)
                ...
                response_trials_direct = fetchn(STIMANAL.ROIResponseDirectVariabilitySpikesPC1removed & key & k_pc,'response_peak_trials');
                idx_photostim_trials_used_direct = fetch1(STIMANAL.ROIResponseDirectVariabilitySpikesPC1removed & key & k_pc,'idx_photostim_trials_used')';
                ...
                idx_weak_direct = response_trials_direct<=weak_response_threshold;
                idx_strong_direct = response_trials_direct>=strong_response_threshold;
                %                 idx_no_response = response_trials_direct<=0.25;
                
                
                response_trace_trials_direct = fetch1(STIMANAL.ROIResponseDirectVariabilitySpikesPC1removed & k_pc & key,'response_trace_trials');
                time_vector_direct = fetch1(STIMANAL.ROIResponseDirectVariabilitySpikesPC1removed & k_pc & key,'time_vector');
                
                
                
                parfor i_r = 1:1:numel(signif_roi)
                    i_r;
                    ...
                    y=response_trace_trials_direct(idx_weak_direct,:);
                    %                     y_smooth=  movmean(mean(y),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    %                     ystem=std(y)./sqrt(size(y,1));
                    %                     ystem_smooth=  movmean(ystem,[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    %
                    ...
                    y=response_trace_trials_direct(idx_strong_direct,:);
                    %                     y_smooth=  movmean(mean(y),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    %                     ystem=std(y)./sqrt(size(y,1));
                    %                     ystem_smooth=  movmean(ystem,[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    %
                    ...
                    photostim_start_frame = photostim_start_frame(idx_photostim_trials_used_direct);
                    try
                        f_trace_coupled= fetch1(IMG.ROISpikesPCremoved & k_pc & k2,'spikes_trace');
                    catch
                        continue
                    end
                    global_baseline=mean( f_trace_coupled);
                    
                    timewind_response = [ 0 2];
                    timewind_baseline1 = [ -5 0];
                    timewind_baseline2  = [-5 0] ;
                    timewind_baseline3  = [ -5 0];
                    [StimStat,StimTrace, idx_photostim_trials_used_coupled] = fn_compute_photostim_response_variability (f_trace_coupled , photostim_start_frame, timewind_response, timewind_baseline1,timewind_baseline2,timewind_baseline3, flag_baseline_trial_or_avg, global_baseline, time);
                    ...
                    %% below 25 response percentile
                    y=StimTrace.response_trace_trials(idx_weak_direct(idx_photostim_trials_used_coupled),:);
                    y_smooth=  movmean(mean(y),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    ystem=std(y)./sqrt(size(y,1));
                    ystem_smooth=  movmean(ystem,[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    ...
                    %% above 75 response percentile
                    y=StimTrace.response_trace_trials(idx_strong_direct(idx_photostim_trials_used_coupled),:);
                    y_smooth=  movmean(mean(y),[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    ystem=std(y)./sqrt(size(y,1));
                    ystem_smooth=  movmean(ystem,[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    %                     max_epoch=max(y_smooth+ystem);
                    ...
                    try
                        r=corr(response_trials_direct(idx_photostim_trials_used_coupled),response_trials_coupled);
                    catch
                        r=NaN;
                    end
